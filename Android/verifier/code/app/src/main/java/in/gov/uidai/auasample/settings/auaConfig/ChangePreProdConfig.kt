...
class ChangePreProdConfig : Fragment() {
...
//        (activity as AppCompatActivity).supportActionBar?.setDisplayHomeAsUpEnabled(true)
        (activity as AppCompatActivity).supportActionBar?.title = "Pre-Prod Configurations"
...
//            }
//        }
//        config_heading.text = "PreProd Configurations"
        ...
        reset_Configs.setOnClickListener {
            ConfigUtils.resetConfig(requireContext(), ConfigUtils.PREPROD_CONFIG_DATA)
//            dismiss()
        }
    }
    ...
    private fun saveConfigsToLocal() {
        if (validateForm()) {
            ...
            ConfigUtils.saveConfigData(configParams, ConfigUtils.PREPROD_CONFIG_DATA)
            ...
            if (fileUri!=null) {
                ...
                if (fileContent != null) {
                    ConfigUtils.saveCertificate(
                        requireContext(),
                        ConfigUtils.PREPROD_CONFIG_DATA,
                        fileContent
                    )
                }
            }
...
//            dismiss()
    ...
    private fun populateConfigFields() {
        val configParams = ConfigUtils.getConfigData(ConfigUtils.PREPROD_CONFIG_DATA)
        if (configParams != null) {
            aua_code.setText(configParams.auaCode)
            aua_license_key.setText(configParams.auaLicenceKey)
            asa_code.setText(configParams.asaCode)
            asa_license_key.setText(configParams.asaLicenceKey)
            ekyc_url.setText(configParams.eKycUrl)
            auth_url.setText(configParams.authUrl)
            signing_cert.setText(configParams.signingCert)
            sub_aua_code.setText(configParams.subAUACode)
            p12_password.setText(configParams.p12Password)
            aua_signing_p12.setText(configParams.signingP12Path)
        }
    }
